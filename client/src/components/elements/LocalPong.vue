...
export default {
  ...
  computed: {
    ...
    rightPaddleTarget() {
      return (
        this.rightPosition +
        (this.isKeyPressed("ArrowUp") ? -this.paddleSpeed : 0) +
        (this.isKeyPressed("ArrowDown") ? this.paddleSpeed : 0)
      );
    },
  },
  methods: {
    ...
    movePaddles() {
      ...
      if (this.rightPosition !== this.rightPaddleTarget) {
        const targetPosition = Math.max(
          60,
          Math.min(840, this.rightPaddleTarget)
        ); // Clamp target position
        this.rightPosition +=
    ...
    gameLoop() {
      if (this.leftScore < 3 && this.rightScore < 3) {
        this.movePaddles();
        this.moveBall();
        if (router.currentRoute._value.path !== "/local") return;
