...
impl Database {
	...
	// Define a function to clear all content from the app_user table
	...
	// Define a function to clear all content from the chat_rooms table
...
use super::*;
	...
	fn clear_tables() -> Result<()> {
		use schema::app_user::dsl::app_user as app_user;
		use schema::chat_rooms::dsl::chat_rooms as chat_rooms;
		use schema::user_room_connection::dsl::user_room_connection as user_room_connection;
		...
		assert_eq!(Ok(0), user_room_connection.count().first::<i64>(&mut db.pool.get()?));
	...
	fn user_fn() -> Result<()> {
		...
		con.transaction::<_, anyhow::Error, _>(|con:  &mut diesel::r2d2::PooledConnection<ConnectionManager<PgConnection>>| {
			...
			assert_eq!(Ok(1), app_user.count().first::<i64>(con));
			assert_eq!("updated_user", app_user.filter(id.eq(1)).first::<User>(con)?.login);
	...
	fn room_fn() -> Result<()> {
		...
		con.transaction::<_, anyhow::Error, _>(|con:  &mut diesel::r2d2::PooledConnection<ConnectionManager<PgConnection>>| {
			...
			db.add_user(&new_user)?;
			assert_eq!(Ok(1), schema::app_user::dsl::app_user.count().first::<i64>(con));
			...
			assert_eq!(Ok(1), chat_rooms.count().first::<i64>(con));
			assert_eq!("Update ChatRoom name", chat_rooms.filter(id.eq(rid)).first::<ChatRoom>(con)?.name);
	...
	fn connection_fn() -> Result<()> {
		...
		con.transaction::<_, anyhow::Error, _>(|con| {
			...
			let rid = db.add_room(&new_room)?;
			assert_eq!(Ok(1), schema::chat_rooms::dsl::chat_rooms.count().first::<i64>(con));
	...
	fn get_connection_fn() -> Result<()> {
		...
		con.transaction::<_, anyhow::Error, _>(|con| {
			...
			assert_eq!(Ok(2), schema::app_user::dsl::app_user.count().first::<i64>(con));
